...
class IdiosyncraticVolatilityFactor(BaseFactor):
    ...
    def compute(self, prices: pd.DataFrame, returns: pd.DataFrame) -> pd.DataFrame:
        ...
        def _idio_vol(window: pd.DataFrame) -> pd.Series:
            resid = window.subtract(market_ret.loc[window.index], axis=0)
            return resid.std() * np.sqrt(252)

        scores = returns.rolling(self.lookback).apply(
            lambda col: col.std(), raw=True
        )
        # subtract market contribution via rolling OLS beta approximation
